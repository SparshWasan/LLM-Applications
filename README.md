...
Collection of Basic Python LLM applications built with the official **Google Gemini SDK** (`google-genai`), **Groq SDK** (`groq`), and **Gradio** (`gradio`) demonstrating practical LLM workflows: prompt engineering, system instructions, persona adoption, multi-language translation, tone modification, automated question generation, custom UI for specific use-cases and tool calling with external APIs.
...
- [Applications Breakdown](#-applications-breakdown)
  ...
  - [13. Weather Assistant using Tool Calling](#13-weather-assistant-using-tool-calling)
...
- **External API**: Using external APIs to get real-time data (e.g., weather, currency exchange rates).
- **Tool Calling**: Using tool calling to get real-time data from external APIs.
...
### 13. Weather Assistant using Tool Calling

- **Path**: `Weather Assistant/app.py`
- **Description**: Gets the current weather of a city using external API.
- **Key Concepts**: Tool Calling, External API integration, using Groq API key and Weather API key.
- **Model**: `llama-3.3-70b-versatile`

## 💻 Tech Stack & Prerequisites

- **Python**: 3.9 or higher
- **SDKs**: `google-genai` (Official Google GenAI SDK), `groq`
- **Environment Management**: `python-dotenv`
- **UI**: `gradio`
- **LLM Engine**: Google Gemini API (`gemini-2.5-flash`), Groq API (`llama-3.3-70b-versatile`)
...
```bash
pip install google-genai python-dotenv gradio groq
```
...
1. Copy the sample environment file:
   ...
   cp .env.example .env
   ```
2. Open `.env` and paste your API key (obtainable from [Google AI Studio](https://aistudio.google.com/) or [Groq](https://groq.com/)) and External Tool API key (obtainable from [OpenWeather API](https://openweathermap.org/api)) :
   ```env
...
│    └── app.py                                   # Weather assistant using tool calling
```
